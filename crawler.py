...
def remove_between(document, start_str, end_str):
    ...
    while start_str in document and end_str in document:
        ...
        end_str_index = document.find(
            end_str, start_str_index) + len(end_str) - 1
        # get new content without from start to end
        document = document[0: start_str_index] + \
            document[end_str_index + 1: len(document)]
    return document
...
def parsePage(website_url):
    ...
    if "title" in str(data):
        ...
        name = str(website_url).split(
            "/")[-2] + "_" + str(website_url).split("/")[-1]
        if "_" == name[-1]:
            name = name[:-1] + ".txt"
        else:
            name = name + ".txt"
